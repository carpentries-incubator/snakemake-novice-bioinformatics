...
> >   hyphens.
> > * Changing the parameters doesn't automatically trigger Snakemake to re-run the rule (remember
> >   it only looks at file modification times) so you need to use `-f` (or `-R` or `-F`) to
> >   force the job to be re-run.
> >
> {: .solution}
{: .challenge}
